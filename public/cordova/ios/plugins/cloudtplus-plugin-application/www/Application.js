cordova.define("cloudtplus-plugin-application.Application", function(require, exports, module) {
	...
	var CHECK = {
        ...
        EnumPrintItemType: {type: "Enum", enum: ["TEXT", "TITLE_VALUE", "NEW_LINE", "SEPERATOR_LINE", "BAR_CODE", "QR_CODE", "IMAGE", "PRINT_NO"]},
        EnumPrintItemAlignment: {type: "Enum", enum: ["CENTER", "LEFT", "RIGHT"]},
        EnumPrintItemFontSize: {type: "Enum", enum: ["SMALL", "MIDDLE", "BIG"]},
        EnumPictureEditType: {type: "Enum", enum: ["crop", "edit"]},
     ...
     * 批量判断文件是否已经下载，通过回调返回一个Object，key为文件远程url，value为文件在本地存储的路径，如果文件还没有下载，则value为null
     * @param {Object} args 参数对象
     * @return {Object} 返回cloudtplus对象
     */
   ...
   applicationExport.showYSYLiveVideo=function(successCallback,errorCallback,options){
       var getValue = argscheck.getValue;
       options = options || {};
       var appkey = getValue(options.appkey, ""),
     ...
     applicationExport.showBlendLiveVideo=function(successCallback,errorCallback,options){
             ...
             return exec(successCallback, errorCallback, "Application", "showBlendLiveVideo", paramerList);
     };
   ...
   var getValue = argscheck.getValue;
   options = options || {};
   var productCode = getValue(options.productCode, ""),
   host=getValue(options.host,""),
   projectId = getValue(options.projectId,""),
   accessToken = getValue(options.accessToken, ""),
   specialDevice = getValue(options.specialDevice,[]),
    ...
    */
     ...
     var getValue = argscheck.getValue;
     options = options || {};
     var resolution = getValue(options.resolution, 2),
     cameraFacing=getValue(options.cameraFacing,0),
     videoFPS = getValue(options.videoFPS,20),
     orientation = getValue(options.orientation, 2),
     definition = getValue(options.definition,1),
     audioEnabled = getValue(options.audioEnabled,true);
     url=getValue(options.url,"");
     var paramerList = [{
     ...
     return exec(successCallback, errorCallback, "Application", "startRtmp", paramerList);
     };
    ...
    */
     applicationExport.startIActiveMeeting=function(successCallback,errorCallback,options){
           ...
           return exec(successCallback, errorCallback, "Application", "startIActiveMeeting", paramerList);
     };
	 ...
	 *  退出页面
	 */
	applicationExport.exitApp = function(successCallback, errorCallback, data) {
		return exec(successCallback, errorCallback, "Application", "exitApp", [data]);
	};
     ...
     *  @param {Boolean} showPanel 当sourceType是CAMERA时，是否在拍照界面的上下方的显示面板，默认值：false
     ...
     *  @param {freeStyleCropEnabled},    当cropRate不为0时，用于控制裁剪筐是否支持拖动 默认为0 不支持 1支持
	 *  @param {Boolean} alwaysSave 总是保存(拍照时，点击取消也保存)，默认值: false
	 *  @param {Boolean} isTemp 不存储，只用于临时使用，默认值: false
     *  @param {Boolean} isContinue 是否拍照完毕并继续拍照.  默认值false
	 */
	applicationExport.getOfflinePicture = function(successCallback, errorCallback, options) {
		...
		var updateFile = function(successCallback, errorCallback, emptyFn, imageUrl, sources, categoryName, opts) {
			...
			for (var i = 0; i < imageUrl.length; i++) {
				...
				items.push({
					...
					sourceFileUrl: url,
                    photoSource: sources ? sources[i] : null,
		...
		var err = checkOptions("getOfflinePicture", opts, {
            ...
            freeStyleCropEnabled:CHECK.NumberDuration,
        ...
        options.showPanel = opts.showPanel;
        delete opts.showPanel;
		...
		if (err) {
			...
			(function(successCallback, errorCallback, emptyFn, opts, alwaysSave) {
				return function(err) {
                    var data = (getType(err) == "Object" || getType(err) == "Array") ? err : false;
                    if (data) {
                        var imageUrl = data;
                        var sources = null;
                        var categoryName = null;
                ...
                showPanel: options.showPanel || false,    // 当sourceType是CAMERA时，是否在拍照界面的上下方的显示面板，默认值：false
                ...
                freeStyleCropEnabled:options.freeStyleCropEnabled||0,
                isContinue:options.isContinue||false,       // 是否保存并继续拍照  .该模式为true 不会显示相册
			}]);
		}
	};
    ...
    applicationExport.editPicture = function(successCallback, errorCallback, options) {
        var err = checkOptions("editPicture", options, {
                               fileUrl: CHECK.StringIsRequired,
                               type: CHECK.EnumPictureEditType
                               });
        if (err) {
            errorCallback && errorCallback.call(this, err);
        }
        else {
            return exec(successCallback, errorCallback, "Application", "editPicture", [options.fileUrl, options.type]);
        }
    };
	/**
	 *  获取离线视频
	 ...
	 *  显示分享菜单
	 *
	 *  @param {Array} platformTypes 分享平台类型，字符串数组，空数组表示分享到所有平台，平台包括："WechatSession"（微信聊天）, "WechatTimeLine"（微信朋友圈）, "WechatFavorite"（微信收藏）, "QQ"（QQ聊天页面）, "Qzone"（QQ空间）,"DingDing"(钉钉)˜, "WhatsApp", "Email", "SMS"
	 *  @param {String} title 标题
   ...
   applicationExport.currentLocation = function(successCallback, errorCallback, options) {
    ...
    applicationExport.countRebar = function(successCallback, errorCallback, options) {
        options = options || "";
        return exec(successCallback, errorCallback, "Application", "countRebar", [options]);
    };
               
    applicationExport.countRebar2 = function(successCallback, errorCallback, user, trackInfo) {
        user = user || "";
        trackInfo = trackInfo || {};
        return exec(successCallback, errorCallback, "Application", "countRebar2", [user, trackInfo]);
    };
    ...
    applicationExport.unzipFile = function(successCallback, errorCallback, options) {
        ...
        if (err) {
            ...
            return exec(successCallback, errorCallback, "Application", "unzipFile", [opts.fileUrl, opts.id]);
        }
    };
